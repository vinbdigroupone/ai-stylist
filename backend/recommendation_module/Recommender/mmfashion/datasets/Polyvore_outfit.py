...
class PolyvoreOutfitDataset(Dataset):
    ...
    def get_single_compatibility_score(self,
                                       ...
                                       use_cuda=True):
        print('\n\n')
        print('***** FUNC: build_from_cfg *****')
        n_items = embeds.size(0)        # why does this embedding have shape (67, 256)
        outfit_score = 0.0
        num_comparisons = 0.0
        for i in range(n_items - 1):
            ...
            for j in range(i + 1, n_items):
                ...
                print('\n')
                print('===== Item pair ID:', item1_id, item2_id)
                ...
                if metric is None:
                    outfit_score += torch.nn.functional.pairwise_distance(
                        embed1, embed2, 2)
                    print('===== Outfit pair score:', outfit_score)
                else:
                    outfit_score += metric(Variable(embed1 * embed2)).data
                num_comparisons += 1
        outfit_score /= num_comparisons
        print('===== Outfit pair score:', outfit_score.item())
